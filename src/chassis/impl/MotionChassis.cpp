...
MotionChassis::MotionChassis(
    ...
    double wheelDiameter, double gearRatio)
...
void MotionChassis::init() {
  ...
  if (m_trackingWheels->center != nullptr)
    m_trackingWheels->center->reset();
  if (m_trackingWheels->left != nullptr)
...
void MotionChassis::brake() { m_robot->brake(); }

double MotionChassis::getLeftVelocity() const { return 0; }
...
void MotionChassis::setDriveExponent(double exponent) { m_exponent = exponent; }
